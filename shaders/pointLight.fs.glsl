...
vec3 blinnPhong(vec3 uKd, vec3 uKs, float uShininess, vec3 uLightPos_vs, vec3 uLightIntensity, vec3 vPosition_vs, vec3 vNormal_vs) {
...
void main() {
    ...
    vec3 fragLighting = blinnPhong(uKd, uKs, uShininess, uLightPos_vs, uLightIntensity, vPosition_vs, vNormal_vs);
